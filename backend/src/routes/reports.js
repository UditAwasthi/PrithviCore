...
router.get('/reports/download', protect, async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=PrithviCore_Report_${type}_${new Date().toISOString().split('T')[0]}.pdf`);
    doc.pipe(res);

    // Header
    doc.rect(0, 0, doc.page.width, 80).fill('#1a5c2a');
    doc.fillColor('#fff').fontSize(22).font('Helvetica-Bold').text('🌱 PrithviCore', 50, 25);
    ...
    doc.fontSize(9).fillColor('#9ca3af')
       .text('Generated by PrithviCore Smart Farming System', 50, doc.page.height - 40, { align: 'center' });
